  ...
  useEffect(() => {
    const fetchStudentOrders = async () => {
      try {
        ...
        // Filtere auf die letzten 3 Tage und heute
        ...
        setError(t('student.foodOrders.errorLoading'));
        setStudentOrders([]);
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-light">
      ...
      <div className="flex-1 p-4 md:p-8 bg-light">
        {/* Desktop Header - mit zentriertem Titel */}
        <header className="relative flex justify-center items-center mb-6 hidden md:flex">
          ...
          <h2 className="text-2xl font-bold text-dark">{t('student.foodOrders.title')}</h2>
        </header>

        {/* Mobile Header mit zentriertem Titel */}
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-20">
            <div className="flex flex-col items-center space-y-4">
              ...
              <p className="text-dark2 font-medium">{t('student.foodOrders.loading')}</p>
            </div>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-primary to-secondary text-white p-6 rounded-xl shadow-md">
              <h3 className="text-xl font-bold mb-4">{t('student.foodOrders.summaryTitle')}</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white bg-opacity-20 p-4 rounded-lg backdrop-blur-sm text-center">
                  <p className="text-xs text-indigo-100">{t('student.foodOrders.stats.totalOrders')}</p>
                  <p className="text-3xl font-bold">{studentOrders.length}</p>
                </div>
                <div className="bg-white bg-opacity-20 p-4 rounded-lg backdrop-blur-sm text-center">
                  <p className="text-xs text-indigo-100">{t('student.foodOrders.stats.totalSpent')}</p>
                  <p className="text-3xl font-bold">${calculateTotalSpent()}</p>
                </div>
                <div className="bg-white bg-opacity-20 p-4 rounded-lg backdrop-blur-sm text-center">
                  <p className="text-xs text-indigo-100">{t('student.foodOrders.stats.avgOrderPrice')}</p>
            ...
            <div className="bg-light rounded-xl shadow-md border border-gray-200 p-6">
              <h3 className="text-xl font-bold mb-6 flex items-center">
                ...
                {t('student.foodOrders.yourRecentOrders')}
              </h3>
              ...
              {orderDates.length > 0 ? (
                  ...
                  {orderDates.map(date => (
                    <div key={date} className="pb-6 border-b border-gray-200 last:border-0">
                      <h4 className={`font-semibold ${isToday(date) ? 'text-primary' : 'text-dark2'} mb-4 flex items-center`}>
                        ...
                        {isToday(date) ? t('student.foodOrders.today') : formatDate(date)}
                      </h4>
                            ...
                            <div className="flex flex-col md:flex-row justify-between">
                              ...
                              <div className="mt-3 md:mt-0 flex flex-col items-end">
                                ...
                                <p className="text-dark2 text-sm mt-1">
                                  {t('student.foodOrders.qty')}: {order.quantity || 1}
                                </p>
                              </div>
                            </div>
                          </div>
                ...
                <div className="text-center py-10">
                  ...
                  <h4 className="text-xl font-medium text-gray-500 mb-2">{t('student.foodOrders.noRecentOrders')}</h4>
                  <p className="text-gray-400">
                    {t('student.foodOrders.noOrdersMessage')}
                  </p>
                </div>
              )}
            </div>
